...
- **Security: Removed all hardcoded tokens** - **BREAKING CHANGE**: All hardcoded OneTrust tokens have been removed from the source code for security reasons. The token **MUST** now be provided via `window.ELECTRO_PRIVACY_TOKEN` (production) or `window.ELECTRO_PRIVACY_TOKEN_STAGING` (staging) before the user opens the "Your Privacy Choices" modal. If the token is not set, the module will log an error and API calls will fail. This is a security requirement - tokens should never be committed to version control. See README.md for setup instructions.
- **UX: Lazy token initialization** - Token validation now happens when the "Your Privacy Choices" modal opens (lazy loading), not at module initialization. This provides more flexibility - the window variable can be set before or after importing the module, as long as it's set before the user opens the modal. This works seamlessly for both Next.js/Vercel and WordPress/Pantheon sites.
...
- **Maintenance: Removed dist from version control** - Added `dist` folder to `.gitignore` to keep git history clean. `dist` is built automatically at install time via `postinstall` script when installing from GitHub, and included in npm packages via `files` field
- **Maintenance: Added postinstall script** - Automatically builds `dist` folder when package is installed from GitHub. The script gracefully handles cases where build dependencies aren't available (won't break installation)
